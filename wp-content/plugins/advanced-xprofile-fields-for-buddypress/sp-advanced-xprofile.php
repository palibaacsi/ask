...
class SP_Advanced_XProfile{
	...
	public $id;
	public function SP_Advanced_XProfile(){
...
<?php
	...
	public function add_bp_field_colums($columns) {
		$user_columns = get_option($this->id.'user_columns');
		if(!empty($user_columns)):
	...
	public function bp_field_column_content($value, $column_name, $user_id) {
		$user_columns = get_option($this->id.'user_columns');
		if(!empty($user_columns)):
...
function sp_advanced_xprofile_initiate(){
	$SP_Advanced_XProfile = new SP_Advanced_XProfile();
}
